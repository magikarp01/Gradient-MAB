...
def getBudget(variances, kroneckers, numSamples):
...
def calcVariance(fVals):
    numer = 0
    avg = sum(fVals)/len(fVals)
    for fVal in fVals:
        numer += (fVal - avg)**2
    return numer / (len(fVals)+1)

def tradOCBASearch(f, k, d, maxBudget, batchSize, numEvalsPerGrad, minSamples,
